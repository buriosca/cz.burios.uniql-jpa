...
import cz.burios.uniql.persistence.exceptions.SessionLoaderException;
import cz.burios.uniql.persistence.exceptions.ValidationException;
import cz.burios.uniql.persistence.internal.databaseaccess.DatabasePlatform;
...
import cz.burios.uniql.persistence.internal.sessions.factories.model.transport.Oc4jJGroupsTransportManagerConfig;
...
import cz.burios.uniql.persistence.platform.server.NoServerPlatform;
...
import cz.burios.uniql.persistence.sessions.Login;
...
import cz.burios.uniql.persistence.sessions.coordination.DiscoveryManager;
...
// $ import cz.burios.uniql.persistence.sessions.coordination.corba.sun.SunCORBATransportManager;
...
import cz.burios.uniql.persistence.sessions.factories.XMLProjectReader;
...
public class SessionsFactory {
    ...
    protected void processLoginConfig(LoginConfig loginConfig, DatasourceLogin login) {
        ...
        // XML Schema default is cz.burios.uniql.persistence.internal.security.JCEEncryptor
    ...
    protected ServerPlatform buildCustomServerPlatformConfig(CustomServerPlatformConfig platformConfig, DatabaseSessionImpl session) {
        ...
        // Server class - XML schema default is cz.burios.uniql.persistence.platform.server.CustomServerPlatform
        String serverClassName = platformConfig.getServerClassName();
        try {
            Class serverClass = m_classLoader.loadClass(serverClassName);
            if (PrivilegedAccessHelper.shouldUsePrivilegedAccess()){
                Constructor constructor = AccessController.doPrivileged(new PrivilegedGetConstructorFor(serverClass, new Class[] { cz.burios.uniql.persistence.sessions.DatabaseSession.class }, false));
                platform = (ServerPlatform)AccessController.doPrivileged(new PrivilegedInvokeConstructor(constructor, new Object[] { session }));
            }else{
                Constructor constructor = PrivilegedAccessHelper.getConstructorFor(serverClass, new Class[] { cz.burios.uniql.persistence.sessions.DatabaseSession.class }, false);
    ...
    protected Sequence buildSequence(SequenceConfig sequenceConfig) {
        ...
        if (sequenceConfig instanceof DefaultSequenceConfig) {
            ...
            try {
                ...
                Class xmlClass = Class.forName("cz.burios.uniql.persistence.eis.adapters.xmlfile.XMLFileSequence");
    ...
    protected void processJNDINamingServiceConfig(JNDINamingServiceConfig namingConfig, TransportManager tm) {
        ...
        // Encryption class - XML Schema default is cz.burios.uniql.persistence.internal.security.JCEEncryptor
    ...
    protected SessionLog buildJavaLogConfig(JavaLogConfig javaLogConfig, AbstractSession session) {
        ...
        try {
            ...
            javaLog = (SessionLog)((Class)conversionManager.convertObject("cz.burios.uniql.persistence.logging.JavaLog", Class.class)).newInstance();
            javaLog.setSession(session);
        } catch (Exception exception) {
            throw ValidationException.unableToLoadClass("cz.burios.uniql.persistence.logging.JavaLog", exception);
        }
