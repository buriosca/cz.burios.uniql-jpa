...
public class RestCollectionAdapterClassWriter implements EclipseLinkClassWriter, Opcodes {
    ...
    private static final String REFERENCE_ADAPTER_SHORT_SIGNATURE = "cz/burios/uniql/persistence/jpa/rs/util/xmladapters/RestCollectionAdapter";
    ...
    public byte[] writeClass(DynamicClassLoader loader, String className) throws ClassNotFoundException {
        ...
        // Another constructor
        mv = cw.visitMethod(ACC_PUBLIC, "<init>", "(Lcz/burios/uniql/persistence/jpa/rs/PersistenceContext;)V", null, null);
        mv.visitCode();
        mv.visitVarInsn(ALOAD, 0);
        mv.visitVarInsn(ALOAD, 1);
        mv.visitMethodInsn(INVOKESPECIAL, REFERENCE_ADAPTER_SHORT_SIGNATURE, "<init>", "(Lcz/burios/uniql/persistence/jpa/rs/PersistenceContext;)V", false);
        mv.visitInsn(RETURN);
        mv.visitMaxs(2, 2);
        mv.visitEnd();
