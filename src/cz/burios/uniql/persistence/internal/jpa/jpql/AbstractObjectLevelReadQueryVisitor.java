...
import cz.burios.uniql.persistence.history.AsOfSCNClause;
...
import cz.burios.uniql.persistence.jpa.jpql.parser.AbstractSelectClause;
...
import cz.burios.uniql.persistence.jpa.jpql.parser.EclipseLinkAnonymousExpressionVisitor;
 ...
 * cz.burios.uniql.persistence.jpa.jpql.parser.Expression JPQL Expression} representing a
 * <b>SELECT</b> query.
 *
...
abstract class AbstractObjectLevelReadQueryVisitor extends AbstractEclipseLinkExpressionVisitor {
	...
	public void visit(AsOfClause expression) {
		...
		cz.burios.uniql.persistence.history.AsOfClause asOfClause;

		if (expression.hasScn()) {
			asOfClause = new AsOfSCNClause(queryExpression);
		}
		else {
			asOfClause = new cz.burios.uniql.persistence.history.AsOfClause(queryExpression);
		}
		...
		if (joinFetches != null ) {
			...
			for (Join joinFetch : joinFetches) {
				...
				if (variableName.equals(joinFetchVariableName)) {
	                cz.burios.uniql.persistence.expressions.Expression queryExpression = null;
	...
	private static class CountFunctionVisitor extends EclipseLinkAnonymousExpressionVisitor {
		...
		/**
		 * Determines whether the single {@link cz.burios.uniql.persistence.jpa.jpql.parser.Expression
	...
	private class JoinVisitor extends AbstractEclipseLinkExpressionVisitor {
		...
		private Expression addNonFetchJoinedAttribute(cz.burios.uniql.persistence.jpa.jpql.parser.Expression expression,
	...
	private class OneToOneSelectedVisitor extends EclipseLinkAnonymousExpressionVisitor {
		...
		/**
		 * Determines whether the visited {@link cz.burios.uniql.persistence.jpa.jpql.parser.Expression
