 ...
 ******************************************************************************/
package cz.burios.uniql.persistence.internal.codegen;
 ...
 * INTERNAL:
 * <p>
 * <b>Purpose</b>: Model a class for code generation purposes.
 *
 * @since TopLink 3.0
...
public class ClassDefinition extends CodeDefinition {
	...
	protected Vector<Object> methods;
	protected Vector<ClassDefinition> innerClasses;

	public ClassDefinition() {
		this.packageName = "";
		this.imports = new Vector<String>(3);
		this.type = CLASS_TYPE;
		this.interfaces = new Vector<String>(3);
		this.attributes = new Vector<AttributeDefinition>();
		this.methods = new Vector<Object>();
		this.innerClasses = new Vector<ClassDefinition>(3);
	}
	...
	private void addImports(Map typeNameMap) {
		for (Iterator shortNameIt = typeNameMap.keySet().iterator(); shortNameIt.hasNext();) {
			...
			for (Iterator packageNameIt = ((Set) typeNameMap.get(shortName)).iterator(); packageNameIt.hasNext();) {
				String packageName = (String) packageNameIt.next();

				if (!packageName.equals(JAVA_LANG_PACKAGE_NAME) && !packageName.equals(getPackageName())
						&& !packageName.equals("")) {
	...
	private void addTypeNamesToMap(HashMap typeNameMap) {
		...
		for (Iterator<Object> i = getMethods().iterator(); i.hasNext();) {
	...
	private void adjustTypeNames(HashMap typeNameMap) {
		...
		for (Iterator<String> i = new Vector<String>(getInterfaces()).iterator(); i.hasNext();) {
	...
	/**
	 * Parses the class definition, pulls out fully qualified class names, adds
	 * imports for them, and un-fully qualifies the class names. - Assumes that no
	 * imports have been previously added. - Assumes that all types have been fully
	 * qualified to start. - Will not unqualify ambiguous classes (java.util.Date
	 * and java.sql.Date). - Will not add imports for java.lang.* - Will not add
	 * imports for classes in the same package. - Will not parse method bodies, but
	 * will unqualify types it finds.
	 *
	 * ?? - Should unqualification occur during writing? That way, reflective
	 * definitions could take advantage.
	 *
	 */
	public void calculateImports() {
		...
		// Go back through class def, pulling out imports and removing package names
		// from
	...
	protected Vector<Object> getMethods() {
		return methods;
	}
	...
	private void setMethods(Vector<Object> methods) {
	...
	protected void sortImports() {
		setImports(new Vector<String>(new TreeSet<String>(getImports())));
	}
	...
	protected void sortMethods() {
		...
		Comparator<Object> comparison = new Comparator() {
		...
		Vector<Object> sortedMethods = new Vector<Object>(getMethods().size());
	...
	public void writeBody(CodeGenerator generator) {
		...
		for (Enumeration<String> interfacesEnum = getInterfaces().elements(); interfacesEnum.hasMoreElements();) {
		...
		for (Enumeration<AttributeDefinition> attributesEnum = getAttributes().elements(); attributesEnum
				.hasMoreElements();) {
			generator.tab();
		...
		for (Enumeration<Object> methodsEnum = getMethods().elements(); methodsEnum.hasMoreElements();) {
		...
		for (Enumeration<ClassDefinition> innerClassesEnum = getInnerClasses().elements(); innerClassesEnum
				.hasMoreElements();) {
