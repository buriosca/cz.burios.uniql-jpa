 ...
 ******************************************************************************/  
package cz.burios.uniql.persistence.sessions.broker;
...
import cz.burios.uniql.persistence.sessions.server.*;
...
public class SessionBroker extends DatabaseSessionImpl {
    ...
    public SessionBroker() {
        super(new cz.burios.uniql.persistence.sessions.DatabaseLogin());
     ...
     */
    public void addDescriptors(cz.burios.uniql.persistence.sessions.Project project) throws ValidationException {
        throw ValidationException.cannotAddDescriptorsToSessionBroker();
    }
    ...
    public boolean containsQuery(String queryName) {
        ...
        if (isClientSessionBroker() && (containsQuery == false)) {
            ...
            while (names.hasNext()) {
                ...
                if (ssession instanceof cz.burios.uniql.persistence.sessions.server.ClientSession) {
     ...
     * @see cz.burios.uniql.persistence.expressions.Expression#hasAsOfClause Expression.hasAsOfClause()
     */
    ...
    public void loginAndDetectDatasource() throws DatabaseException {
        ...
        for (Iterator sessionEnum = getSessionsByName().values().iterator();
            ...
            DatabaseSessionImpl session = (DatabaseSessionImpl)sessionEnum.next();
            if (!session.isConnected()) {
                if(session.getDatasourcePlatform().getClass().getName().equals("cz.burios.uniql.persistence.platform.database.DatabasePlatform")) {
     ...
     * DatabaseSession/ServerSession should not be connected and descriptors should not be initialized.
     */
    public void registerSession(String name, cz.burios.uniql.persistence.sessions.Session session) {
