...
import cz.burios.uniql.persistence.internal.descriptors.changetracking.AggregateAttributeChangeListener;
...
//    }
        ...
        if (getReferenceClassName() != null) {
            try {
                if (PrivilegedAccessHelper.shouldUsePrivilegedAccess()) {
                    ...
                    setReferenceClass(cz.burios.uniql.persistence.internal.security.PrivilegedAccessHelper.getClassForName(getReferenceClassName(), true, classLoader));
                }
    ...
    protected void executeEvent(int eventCode, ObjectLevelModifyQuery query) {
        ...
        if (referenceDescriptor.getEventManager().hasAnyEventListeners()) {
            referenceDescriptor.getEventManager().executeEvent(new cz.burios.uniql.persistence.descriptors.DescriptorEvent(eventCode, query));
        }
    }
            ...
            if ( newValue == null ) { // attribute set to null
                ...
                for(DatabaseMapping mapping : referenceDescriptor.getMappings()) {
                    if(mapping.isAggregateObjectMapping()) {
                        ...
                        mapping.updateChangeRecord(newValue, nestedNewValue, nestedOldValue, (cz.burios.uniql.persistence.internal.sessions.ObjectChangeSet)changeRecord.getChangedObject(), uow);
                    }
                }
